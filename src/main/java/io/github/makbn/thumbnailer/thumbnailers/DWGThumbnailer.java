...
public class DWGThumbnailer extends AbstractThumbnailer {
  ...
  public void generateThumbnail(File input, File output, ResizeParameters params)
    ...
    try {
      ...
      int PosSentinel = (((outputByte[3]) & 0xFF) * 256 * 256 * 256) + (((outputByte[2]) & 0xFF) * 256 * 256)
          + (((outputByte[1]) & 0xFF) * 256) + ((outputByte[0]) & 0xFF);
      ...
      if (TypePreview == 2) {
        ignored = fis.read(outputByte, 0, 4);
        int PosBMP = (((outputByte[3]) & 0xFF) * 256 * 256 * 256) + (((outputByte[2]) & 0xFF) * 256 * 256)
            + (((outputByte[1]) & 0xFF) * 256) + ((outputByte[0]) & 0xFF);
        ignored = fis.read(outputByte, 0, 4);
        int LenBMP = (((outputByte[3]) & 0xFF) * 256 * 256 * 256) + (((outputByte[2]) & 0xFF) * 256 * 256)
            + (((outputByte[1]) & 0xFF) * 256) + ((outputByte[0]) & 0xFF);
