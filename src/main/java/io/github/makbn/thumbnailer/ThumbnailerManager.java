...
/**
 * This class manages all available Thumbnailers. Its purpose is to delegate a File to the appropriate Thumbnailer in
 * order to get a Thumbnail of it. This is done in a fall-through manner: If several Thumbnailer can handle a specific
 * filetype, all are tried until a Thumbnail could be created.
 * <p>
 * Fill this class with available Thumbnailers via the registerThumbnailer()-Method. Then call generateThumbnail().
 *
 * @author Benjamin
 */
public class ThumbnailerManager implements Thumbnailer {
  ...
  /**
   * Set the folder where the thumbnails should be generated by default (if no output file is given).
   *
   * @param thumbnailPath
  ...
  /**
   * Generate a Thumbnail. The output file name is generated using a hashing scheme. It is garantueed that an existing
   * Thumbnail is not overwritten by this.
   *
   * @param input
   ...
   */
  public File createThumbnail(File input, String ext, ResizeParameters params)
      throws FileDoesNotExistException, IOException, ThumbnailerException {
  ...
  /**
   * Add a Thumbnailer-Class to the list of available Thumbnailers Note that the order you add Thumbnailers may make a
   * difference: First added Thumbnailers are tried first, if one fails, the next (that claims to be able to treat such
   * a document) is tried. (Thumbnailers that claim to treat all MIME Types are tried last, though.)
   *
   * @param thumbnailer
   *          Thumbnailer to add.
   */
   ...
   * <p>
   * This functions should be called before termination of the program, and Thumbnails can't be generated after calling
   * this function.
   */
  @Override
  public void close() {
  ...
  /**
   * Generate a Thumbnail of the input file. Try all available Thumbnailers and use the first that returns an image.
   * <p>
   ...
   * @throws ThumbnailerException
   *           If the thumbnailing process failed (i.e., no thumbnailer could generate an Thumbnail. The last
   *           ThumbnailerException is re-thrown.)
   * @param mimeType
  ...
  public void generateThumbnail(File input, File output, String mimeType, ResizeParameters params)
    ...
    if (mimeType != null)
      generated = executeThumbnailers(mimeType, input, output, mimeType, params);

    // Try again using wildcard thumbnailers
    if (!generated)
      generated = executeThumbnailers(ALL_MIME_WILDCARD, input, output, mimeType, params);

    if (!generated)
      throw new ThumbnailerException(
          "No suitable Thumbnailer has been found. (File: " + input.getName() + " ; Detected MIME: " + mimeType + ")");
  }
  ...
  public void generateThumbnail(File input, File output, ResizeParameters params)
      throws IOException, ThumbnailerException {
    generateThumbnail(input, output, null, params);
  }
   ...
   * @throws IOException
   *           Input file cannot be read, or output file cannot be written, or necessary temporary files could not be
   *           created.
   * @return True on success (1 thumbnailer could generate the output file).
   */
  private boolean executeThumbnailers(String useMimeType, File input, File output, String detectedMimeType,
      ResizeParameters params) throws IOException {
    for (Thumbnailer thumbnailer : thumbnailers.getIterable(useMimeType)) {
      try {
        thumbnailer.generateThumbnail(input, output, detectedMimeType, params);
        return true;
  ...
  public void setImageSize(int imageResizeOptions) {
    for (Thumbnailer thumbnailer : allThumbnailers)
      thumbnailer.setImageSize(imageResizeOptions);
  }
